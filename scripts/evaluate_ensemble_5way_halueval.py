"""
5-way ensemble evaluation: NLI + Self-Consistency + Semantic Similarity +
Uncertainty + Retrieval-augmented Verification on HaluEval.
Extends evaluate_ensemble_4way_halueval.py with the retrieval verification detector.
Ground truth: token-overlap F1 proxy against HaluEval's reference_answer.
"""
...
from detection.uncertainity_methods import UncertaintyDetector
from detection.retrieval_verification import RetrievalVerificationDetector
...
def main():
    ...
    rv_detector = RetrievalVerificationDetector(nli_detector=nli_detector)
    ...
    for i, row in df.iterrows():
        ...
        unc_result = unc_detector.check(llm, row["context"], row["input_text"], main_answer)
        rv_result = rv_detector.check(context=row["context"], claim=main_answer)
        ...
        rows.append({
            ...
            "unc_risk": unc_result["hallucination_risk"],
            "rv_risk": rv_result["hallucination_risk"],
        })

    out_df = pd.DataFrame(rows)

    for col in ["nli_risk", "sc_risk", "sem_risk", "unc_risk", "rv_risk"]:
    ...
    out_df["ensemble_nli_sc_unc"] = (out_df["nli_risk_norm"] + out_df["sc_risk_norm"] + out_df["unc_risk_norm"]) / 3
    out_df["ensemble_nli_sc_rv"] = (out_df["nli_risk_norm"] + out_df["sc_risk_norm"] + out_df["rv_risk_norm"]) / 3
    out_df["ensemble_nli_sc_unc_rv"] = (out_df["nli_risk_norm"] + out_df["sc_risk_norm"] + out_df["unc_risk_norm"] + out_df["rv_risk_norm"]) / 4
    out_df["ensemble_5way"] = (out_df["nli_risk_norm"] + out_df["sc_risk_norm"] + out_df["sem_risk_norm"] + out_df["unc_risk_norm"] + out_df["rv_risk_norm"]) / 5

    out_df.to_csv("results/logs/ensemble_5way_halueval_eval.csv", index=False)

    methods = ["nli_risk", "sc_risk", "sem_risk", "unc_risk", "rv_risk",
               "ensemble_2way", "ensemble_nli_sc_unc", "ensemble_nli_sc_rv",
               "ensemble_nli_sc_unc_rv", "ensemble_5way"]
    ...
    print("\nSaved detailed results to results/logs/ensemble_5way_halueval_eval.csv")
