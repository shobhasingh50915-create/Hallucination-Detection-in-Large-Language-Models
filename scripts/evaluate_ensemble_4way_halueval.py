"""
4-way ensemble evaluation: NLI + Self-Consistency + Semantic Similarity + Uncertainty on HaluEval.
Extends evaluate_ensemble_3way_halueval.py with the verbalized-confidence uncertainty detector.
...
def main():
    ...
    for col in ["nli_risk", "sc_risk", "sem_risk", "unc_risk"]:
    ...
    out_df["ensemble_4way"] = (out_df["nli_risk_norm"] + out_df["sc_risk_norm"] + out_df["sem_risk_norm"] + out_df["unc_risk_norm"]) / 4

    out_df.to_csv("results/logs/ensemble_4way_halueval_eval.csv", index=False)

    for method in ["nli_risk", "sc_risk", "sem_risk", "unc_risk", "ensemble_2way", "ensemble_3way", "ensemble_nli_sc_unc", "ensemble_4way"]:
        print(f"\n=== {method} ===")
    ...
    print("\nSaved detailed results to results/logs/ensemble_4way_halueval_eval.csv")
